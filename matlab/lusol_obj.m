...
%              end
            %fprintf('Swap: %d\n', i);
             if mod(i,maxswaps+1) == 0
                 obj.refactor('pivot', 'TRP','Ltol1',1.4, 'nzinit', 5*10^6)
             end
...
%              S = A22 - obj.A21*obj.A11^-1*obj.A12;          
%              [~, s_cT] = max(sqrt(sum(S.^2)));
%              [alphaT,~] = max(abs(S(:,s_cT)));
%              
...
%             A22 = obj.A(obj.ap(nrank+1:end),obj.aq(nrank+1:end));
%             S2 = obj.O*(A22 - obj.A21*(obj.A11\obj.A12));
%             errors(i) = norm(S2-obj.S,'fro')/norm(S2,'fro');
    ...
    function [err] = CURerror(obj)
        nrank = obj.stats.nrank;
        C = obj.A(obj.ap(1:end),obj.aq(1:nrank));
        R = obj.A(obj.ap(1:nrank),obj.aq(1:end)); 
        
        %P = obj.Qc'*obj.Apq*obj.Qr'; 
